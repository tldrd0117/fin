...
import numpy as np
import logging
logging.basicConfig(handlers=[logging.FileHandler('simulation22.log', 'w', 'utf-8')], level=logging.INFO, format='%(message)s')
...
endDate = pd.to_datetime('2019-10-28', format='%Y-%m-%d')
...
while endDate >= current:
    ...
    if nextInvestDay <= current or nextInvestDay.day == current.day:
        ...
        # target = ss.getCurValuePerStockNumFactor(current, topdf[target], factordf, '당기순이익', marcapdf, sCode, sName, 1000, True, int(len(target)/2), minVal=0.00000001)
        target = ss.getCurValuePerStockNumFactor(current, topdf[target], factordf, '당기순이익', marcapdf, sCode, sName, 1000, True, int(len(target)/2), minVal=0.00000001)
        target = ss.getCurValuePerStockNumFactor(current, topdf[target], factordf, '영업활동으로인한현금흐름', marcapdf, sCode, sName, 1000, True, 100, minVal=0.00000001)
        ...
        target = ss.getFactorPerStockNum(current, topdf[target], factordf, '영업활동으로인한현금흐름', marcapdf, sCode, sName, False, 100, minVal=0.00000001)
        target = ss.getFactorList(current, topdf[target], factordf, '영업활동으로인한현금흐름',sName, sCode, False, 100, minVal=0.00000001)
        ...
        target = ss.getVPCIUpListLimit(current, topdf[target], amountdf[target], 30)
    ...
    lists = ss.getVPCIDownList(current, topdf[lossstocks], amountdf[lossstocks])
    for item in lists:
