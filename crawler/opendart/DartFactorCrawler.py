...
import pandas as pd
import logging
logging.basicConfig(handlers=[logging.FileHandler('factorTest2020.log', 'w', 'utf-8')], level=logging.INFO, format='%(message)s')
...
class DartFactorCrawler:
    ...
    codes = [{"자산":"ifrs-full_Assets"},
        ...
        {"자본총계":["ifrs-full_Equity","ifrs_Equity"]},
        {"영업이익":"dart_OperatingIncomeLoss"},
        {"매출액":["ifrs-full_Revenue","ifrs-full_GrossProfit","매출액","영업수익"]},
        {"매출총이익":["ifrs-full_GrossProfit"]},
        {"영업활동으로인한현금흐름":["ifrs-full_CashFlowsFromUsedInOperatingActivities","ifrs_CashFlowsFromUsedInOperatingActivities", '영업활동으로 인한 순현금흐름','영업으로부터 창출된 현금흐름','영업활동순현금흐름','dart_AdjustmentsForAssetsLiabilitiesOfOperatingActivities']},
        {"투자활동으로인한현금흐름":["ifrs-full_CashFlowsFromUsedInInvestingActivities",'ifrs_CashFlowsFromUsedInInvestingActivities','투자활동으로 인한 순현금흐름']},
        {"재무활동으로인한현금흐름":["ifrs-full_CashFlowsFromUsedInFinancingActivities",'ifrs_CashFlowsFromUsedInFinancingActivities','재무활동으로 인한 순현금흐름']},
        {"당기순이익":["ifrs-full_ProfitLoss",'ifrs_ProfitLoss','dart_ProfitLossForStatementOfCashFlows']},
        {"이자비용":["ifrs-full_InterestPaidClassifiedAsOperatingActivities","ifrs-full_InterestPaidClassifiedAsFinancingActivities",'dart_AdjustmentsForInterestExpenses']},
        {"법인세비용":["ifrs-full_IncomeTaxExpenseContinuingOperations",'ifrs_IncomeTaxExpenseContinuingOperations', 'ifrs-full_AdjustmentsForIncomeTaxExpense']}
        ]
    ...
    def getCsvResultName(self, year):
        return f'finData/{str(year)}/factors.csv'
    
    def getHdfResultName(self, year):
        return f'h5data/FACTORS_{str(year)}-12-30.h5'
    ...
    def getFactors(self, year):
        ...
        try:
            ...
            df.to_csv(self.getCsvResultName(year))
            df.to_hdf(self.getHdfResultName(year), key='df', mode='w')
            print("complete!!")
    ...
    def getFactor(self, year, stock_code, codeDict):
        ...
        for key in self.columns:
            if not key in resultDict or resultDict[key] == "" or not resultDict[key]:
                resultDict[key] = "0"
        if float(resultDict["영업활동으로인한현금흐름"]) == 0:
            printG(result)
